    ...
    if (Smarty::$_MBSTRING) {
        ...
        if (mb_strlen($string, Smarty::$_CHARSET) > $length) {
        	if ($break_words && $middle) {
        		list($string, $w) = explode($break_words, $string);
        		return mb_substr($string, 0, 2 , Smarty::$_CHARSET) . $etc . $break_words . $w;
			}
			if (!$break_words && $middle) {
        		return mb_substr($string, 0, 2 , Smarty::$_CHARSET) . $etc . mb_substr($string, -2, 2, Smarty::$_CHARSET);;
			}
