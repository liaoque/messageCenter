...
function _param_handle($source, $def, $prefix = null, &$scope = null, $default = array()) {
	...
	$_PARAM_NULLOK = $PARAM_UINT ^ $PARAM_SINT ^ $PARAM_FLOAT ^ $PARAM_BOOL ^ $PARAM_HEX ^ $PARAM_ENUM_A ^ $PARAM_ENUM_B ^ $PARAM_ENUM_C ^ $PARAM_REGEXP;
	...
	foreach ($def as $var => $flags) {
		...
		if (isset($source[$sourcevar])) { // do no change without accounting for $PARAM_EXISTS
			...
			if ($flags &$PARAM_ARRAY) {
                ...
                $scope[$sourcevar]=$ref;continue;
                //TODO:WCZ:获取数组的过程中，暂时屏蔽对数组内元素的格式验证
				/*if (is_array($ref)) {
			...
			// enum
			if ($flags &$PARAM_ENUM || $flags &$PARAM_ENUM_A || $flags &$PARAM_ENUM_B || $flags &$PARAM_ENUM_C
				) {
				$enums = array();
			...
			if ($flags &$PARAM_UINT) { // unsigned (non-negative integer)
				...
				if ($flags &$PARAM_STRIPTAGS) {
					...
					if ($allowtags && strpos($ref, '=') !== false) { // is there an entitity we need to protect?
						...
						$exprs = array('/( on[a-z]{1,}|style|class|id|target|print|alert)="(.*?)"/i',
				...
				$ref = str_replace(array("\r", "print", "alert", 'print_r', 'printf'), array(""), $ref);
