...
final class jPushDrive_Http {

    public static function get($client, $url) {
        $response = self::sendRequest($client, $url, jPushDrive_Config::HTTP_GET, $body=null);
        return self::processResp($response);
    }
    public static function post($client, $url, $body) {
        $response = self::sendRequest($client, $url, jPushDrive_Config::HTTP_POST, $body);
        return self::processResp($response);
    }
    public static function put($client, $url, $body) {
        $response = self::sendRequest($client, $url, jPushDrive_Config::HTTP_PUT, $body);
        return self::processResp($response);
    }
    public static function delete($client, $url) {
        $response = self::sendRequest($client, $url, jPushDrive_Config::HTTP_DELETE, $body=null);
        return self::processResp($response);
    }
    ...
    private static function sendRequest($client, $url, $method, $body=null, $times=1) {
        ...
        curl_setopt($ch, CURLOPT_USERAGENT, jPushDrive_Config::USER_AGENT);
        curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, jPushDrive_Config::CONNECT_TIMEOUT);  // 连接建立最长耗时
        curl_setopt($ch, CURLOPT_TIMEOUT, jPushDrive_Config::READ_TIMEOUT);  // 请求最长耗时
        ...
        // 设置Post参数
        if ($method === jPushDrive_Config::HTTP_POST) {
            curl_setopt($ch, CURLOPT_POST, true);
        } else if ($method === jPushDrive_Config::HTTP_DELETE || $method === jPushDrive_Config::HTTP_PUT) {
