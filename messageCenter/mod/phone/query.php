 ...
 * Time: 10:14
 */
class Phone_query extends ProMessageCenter_MessageCheck
{
    ...
    public function indexFunc()
    {
        ...
        $result = DbMessageCenter_LogPhone::getInstance()->find([
            'pkId' => $info['pkId']
        ], 'status');
        if (empty($result)) {
            show_message(DbMessageCenter_LogPhone::$status[DbMessageCenter_LogPhone::STATUS_SEND_ING], '', 1);
        }
        show_message(DbMessageCenter_LogPhone::$status[$result['status']], '', 1);
